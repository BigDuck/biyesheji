...
/**
 * Name：SystemOSUtils
 * Time：2016/1/23 16:52
 * author：WPJ587
 * description：系统工具类 部分代码来自网络
 **/
...
public class SystemOSUtils {
     ...
     */
    public static Map<String, String> getOSNetInfo() throws SigarException {
        ...
        for (int i = 0; i < ifaces.length; i++) {
            ...
            OSInfo.put("address",cfg.getAddress());//IP地址
            OSInfo.put("broadcast",cfg.getBroadcast());//网关广播地址
            OSInfo.put("hwaddr", cfg.getHwaddr());//网卡MAC地址
            OSInfo.put("netmask", cfg.getNetmask());//子网掩码
            OSInfo.put("description", cfg.getDescription());//网卡描述信息
            OSInfo.put("netType", cfg.getType());//网卡类型
     ...
     * @throws Exception
     */
    public static List<String> getfile() throws Exception {
//        Sigar sigars = new Sigar();
        List<String> diskMsg=new ArrayList<>();
        FileSystem fslist[] = sigar.getFileSystemList();
        for (int i = 0; i < fslist.length; i++) {
            diskMsg.add("分区的盘符名称" + i);
            FileSystem fs = fslist[i];
            diskMsg.add("盘符名称:" + fs.getDevName());
            diskMsg.add("盘符路径:" + fs.getDirName());
            diskMsg.add("盘符标志:" + fs.getFlags());
            diskMsg.add("盘符类型:" + fs.getSysTypeName());
            diskMsg.add("盘符类型名:" + fs.getTypeName());
            diskMsg.add("盘符文件系统类型:    " + fs.getType());
            ...
            switch (fs.getType()) {
                ...
                case 2: // TYPE_LOCAL_DISK : 本地硬盘
                    // 文件系统总大小
                    diskMsg.add(fs.getDevName() + "总大小:    " + usage.getTotal() + "KB");
                    // 文件系统剩余大小
                    diskMsg.add(fs.getDevName() + "剩余大小:    " + usage.getFree() + "KB");
                    // 文件系统可用大小
                    diskMsg.add(fs.getDevName() + "可用大小:    " + usage.getAvail() + "KB");
                    // 文件系统已经使用量
                    diskMsg.add(fs.getDevName() + "已经使用量:    " + usage.getUsed() + "KB");
                    double usePercent = usage.getUsePercent() * 100D;
                    // 文件系统资源的利用率
                    diskMsg.add(fs.getDevName() + "资源的利用率:    " + usePercent + "%");
                    break;
            ...
            diskMsg.add(fs.getDevName() + "读出：    " + usage.getDiskReads());
            diskMsg.add(fs.getDevName() + "写入：    " + usage.getDiskWrites());
